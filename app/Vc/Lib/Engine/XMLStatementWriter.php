...
use App\Lib\Base;
use App\Vc\Lib\Engine\Data\DynamicSequenceValue;
    ...
    public function parseToDVData(string $p_stmt){
        ...
        if(count($l_dvData)==1){
            if($l_dvData[0][0]==DynamicSequenceValue::TYPE_STRING){
                return "new \\App\\Vc\\Lib\\Engine\\Data\\DynamicStaticValue(".var_export($l_dvData[0][1],true).")";
            }else if($l_dvData[0][0]==DynamicSequenceValue::TYPE_VAR){
                return "new \\App\\Vc\\Lib\\Engine\\Data\\DynamicVarValue(".var_export($l_dvData[0][1],true).")";
            } else {
                throw new XMLParserException(__("Invalid value tpe code :   code",$l_dvData[0][0]),null);
            }
        } else {
            return "new \\App\\Vc\\Lib\\Engine\\Data\\DynamicSequenceValue(".var_export($l_dvData,true).")";
        }
    }
