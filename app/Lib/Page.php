...
<div class="topMenu">
    ...
    {
        ?>
		<span class="topMenuItem"><a class='topMenuLink'
		href='<?=route($p_route,$p_parameters)?>'><?=htmlspecialchars($p_title)?></a></span>
	<?php
    }
    ...
    static function topMenuItemConfirm($p_route, Array $p_parameters, $p_title, $p_message)
    {
        $l_js = self::confirmJs($p_message, route($p_route, $p_parameters))?>
			<span class="topMenuItem"><a class='topMenuLink' href='#'
		onclick='<?=self::e($l_js)?>'><?=htmlspecialchars($p_title)?></a></span>
		<?php
    }
