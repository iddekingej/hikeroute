...
class Magic{
	 ...
	 * @return boolean
	 */
	private function comp(Array $p_data,string $p_str)
	{
		$l_end=count($p_data);
		for($l_cnt=0;$l_cnt<$l_end;$l_cnt++){
			if(chr($p_data[$l_cnt]) != $p_str[$l_cnt]) return false;
		}
		return true;
	}
	...
	function findMimeByMagick($p_file):?string
	{
		...
		if($l_fileHandle===false){
		    throw new Exception("File $p_file not found");
		}
		$l_dataZero=NULL;
		foreach(self::$data as $l_key=>$l_info){
			if($l_info[0]){
				...
				if(count($l_bin)<=strlen($l_data)){
					if($this->comp($l_bin,$l_data)){
						fclose($l_fileHandle);
						return $l_key;	
					}
				}
