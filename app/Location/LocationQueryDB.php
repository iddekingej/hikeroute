...
class LocationQueryDB implements LocationQueryService
{
	...
	{
		$l_locations=\DB::connection("locationdb")->select(\DB::raw("select admin_level,name from planet_osm_polygon where ST_CONTAINS(way,ST_TRANSFORM(ST_GeomFromText('point('||:lon||' '||:lat||')',4326),3857)) and admin_level in ('2','4','8','10') "),["lon"=>$p_lon,"lat"=>$p_lat]);
